...
class SwipeController(var buttonsActions:SwipeControllerActions): ItemTouchHelper.Callback() {
    ...
    override fun onChildDraw(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean
        ...
        if (actionState == ACTION_STATE_SWIPE) {
            if (buttonShowedState != ButtonsState.GONE) {
                var tmpDX:Float = 0f
                if (buttonShowedState == ButtonsState.LEFT_VISIBLE) tmpDX =
                    Math.max(dX, buttonWidth)
                if (buttonShowedState == ButtonsState.RIGHT_VISIBLE) tmpDX =
                    Math.min(dX, -buttonWidth)
                super.onChildDraw(c, recyclerView, viewHolder, tmpDX, dY, actionState, isCurrentlyActive
                )
            } else {
